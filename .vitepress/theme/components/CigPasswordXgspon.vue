<template>
  <div>
    <form id="cig-password-xgspon" novalidate @submit.prevent="handleSubmit">
      <div class="form-floating mb-3">
        <label for="serial" class="form-label">XGS-PON S/N in format XGSPabc12345</label>
        <input
          v-model="serial"
          type="text"
          class="form-control"
        ...
        />
        <div class="invalid-feedback">Please provide a valid XGS-PON S/N.</div>
      </div>
      <div class="mb-3">
...
function cigpassword_xgspon(ont_serial: string, ont_user: string): string {
  ...
  let ont_id = ont_serial.substring(4).toUpperCase()
...
function handleSubmit(event: Event) {
  ...
  if (form.checkValidity()) {
    result.value = cigpassword_xgspon(serial.value, props.username || '')
  }
